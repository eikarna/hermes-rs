...
- Memory Firewall (`[memory]`, Task t_f7c40431): trust tagging per provenance source (`user`/`agent`/`file`/`tool`/`web`), secret masking on store (`redaction.rs`), quarantine of low-trust memories, and backward-compatible disk serialization/migration
...
- (none — all tracked items implemented)
