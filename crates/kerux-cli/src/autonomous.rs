...
fn build_autonomous_query(
    ...
    format!(
        "You are running in kerux autonomous coding mode.\n\
...
mod tests {
    ...
    fn sample_config() -> AppConfig {
        ...
        config.autonomous.commit_message = "Auto-commit by kerux".to_string();
    ...
    fn init_disposable_repo(todo_contents: &str) -> PathBuf {
        ...
        git_success(&repo_root, &["config", "user.name", "Kerux Test"]);
        git_success(&repo_root, &["config", "user.email", "kerux@example.com"]);
