...
mod tests {
    ...
    fn extracts_rust_function_definition() {
        let dir = std::env::temp_dir().join(format!("hermes_repomap_{}", std::process::id()));
    ...
    fn discovers_and_skips_target_dir() {
        let dir = std::env::temp_dir().join(format!("hermes_repomap_disc_{}", std::process::id()));
    ...
    fn extracts_python_def() {
        let dir = std::env::temp_dir().join(format!("hermes_repomap_py_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
