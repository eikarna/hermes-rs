...
//! a data directory under `~/.kerux/`, filesystem-safe keys, and JSON
...
/// Root data directory: `~/.kerux/<subdir>`.
///
/// The `KERUX_HOME` env var overrides the `~/.kerux` root entirely
/// (useful for tests and portable installs). Falls back to
/// `./.kerux/<subdir>` when the home dir is unavailable.
pub fn data_root() -> PathBuf {
    if let Some(home) = std::env::var_os("KERUX_HOME") {
