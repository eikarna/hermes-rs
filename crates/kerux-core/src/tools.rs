...
pub use builtin::{
    builtin_tool_names, register_builtin_tools, register_builtin_tools_with_provider_sub_agent,
    register_builtin_tools_with_sub_agent, ClarifyTool, CodeExecutionTool, DateTimeTool,
...
/// Trait for defining a Kerux tool
///
/// Tools must provide:
/// - A unique name
...
pub struct ToolRegistry {
    tools: Arc<RwLock<HashMap<String, Arc<dyn KeruxTool>>>>,
...
impl ToolRegistry {
    ...
    pub async fn register<T: KeruxTool + 'static>(&self, tool: T) -> Result<()> {
    ...
    /// Get a tool by name
    pub async fn get(&self, name: &str) -> Option<Arc<dyn KeruxTool>> {
        let tools = self.tools.read().await;
