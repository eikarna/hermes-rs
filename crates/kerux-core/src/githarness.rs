...
impl GitHarness {
    ...
    pub fn commit_transaction(&self, fallback_subject: &str) -> Result<Option<String>> {
        ...
        let tmp = std::env::temp_dir().join(format!("kerux-commit-msg-{}", std::process::id()));
    ...
    pub fn undo(&self, snapshot: &RepoSnapshot) -> Result<()> {
        ...
        if !snapshot.dirty_patch.is_empty() {
            ...
            let tmp = std::env::temp_dir().join(format!("kerux-undo-{}.patch", std::process::id()));
...
mod tests {
    ...
    fn init_repo() -> PathBuf {
        ...
        git(&dir, &["init"]);
        git(&dir, &["config", "user.name", "Kerux Test"]);
        git(&dir, &["config", "user.email", "kerux@example.com"]);
