...
**Contributors:** Alxy Savin ([@xsa-dev](https://github.com/xsa-dev))
...
- **Richer telemetry HUD** (Tasks 3.1–3.4): provider-reported usage now drives the TUI status line — tokens/sec, turns completed, context-window usage %, and cached prompt tokens (OpenAI `prompt_tokens_details.cached_tokens`, Anthropic `cache_read_input_tokens`, Gemini `cachedContentTokenCount`); streaming requests opt into `stream_options.include_usage` so streamed runs report real usage instead of estimates, and provider-quoted costs (`estimated_cost_usd`) take precedence over local price estimates. Gateway live-status lines surface tok/s and cache hits too.
- **Cost guardrail enforcement in the agent loop**: `[budget]` ceilings are now enforced per turn — usage is recorded after every LLM response, the warn threshold emits a one-time `BudgetAlert`, `on_limit = pause|stop` halts the run with `BudgetExceeded`, and `on_limit = downgrade` switches the rest of the run to `downgrade_model` (once); per-turn estimated cost feeds `AgentTelemetry.estimated_cost_usd`, gateway/TUI surfaces render budget alerts, and autonomous mode carries the daily accumulator across ticks
